...
uint32_t CommandContext::ReadbackTexture(ReadbackBuffer& DstBuffer, PixelBuffer& SrcBuffer)
{
    ...
    ID3D12Resource* dstBufferResource = DstBuffer.GetResource();
    ID3D12Resource* srcBufferResource = SrcBuffer.GetResource();

    D3D12_TEXTURE_COPY_LOCATION dstCopyLocation = 
        CD3DX12_TEXTURE_COPY_LOCATION(dstBufferResource, PlacedFootprint);
    D3D12_TEXTURE_COPY_LOCATION srcCopyLocation =
        CD3DX12_TEXTURE_COPY_LOCATION(srcBufferResource, 0);

    m_CommandList->CopyTextureRegion(&dstCopyLocation, 0, 0, 0, &srcCopyLocation, nullptr);
